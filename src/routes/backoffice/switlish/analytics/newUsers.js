 ...
 */
const settingsRoute = {
  method: 'get', path: 'bo/switlish/analytics/new-users'
};
const middlewares = [
  authorizeClient,
  ...
  async process() {
    ...
    const dataset = await User.aggregate()
      .match({
        ...
        signed_up_at: { $gte: this.period.startDate, $lte: this.period.endDate }
      })
      .group({
        _id: { $dateToString: { date: '$signed_up_at', format: '%Y-%m-%d', timezone: this.timezone } },
        value: { $sum: 1 }
      })
      .sort({ _id: 'ascending' })
