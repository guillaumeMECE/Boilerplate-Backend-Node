...
const widgetUsersCountRouteController = buildRouteCallback(`${__dirname}/widgetUsersCount`);
...
describe('[Switlish > Metrics] - widgetUsersCount', () => {
    // Seeds
    ...
    beforeEach(async () => {
        await clearDatabase();
        try {
            const client = await createSeed.createClient();
            const website = await createSeed.createWebsite();
            await createSeed.createClientWebsite(client._id, website._id);
        } catch (error) {
    ...
    describe('[CASE-0] req with valid header', () => {
        it('should return status(200) ', async () => {
            ...
            // Act
            try {
                await widgetUsersCountRouteController(req, res);
            } catch (error) {
